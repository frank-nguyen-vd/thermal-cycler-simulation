...
class MachineLearning:
    ...
    def train_model(self, train_path=None):
        ...
        def baseline_model():
            model = Sequential()
            model.add(Dense(1000, input_dim=5, kernel_initializer='truncated_normal', activation='relu'))
            model.add(Dropout(0.1))
            model.add(Dense(1, kernel_initializer='truncated_normal'))
            model.compile(loss='mean_squared_error', optimizer='sgd')
            return model
        X_train, y_train = self.load_data(train_path)
        # evaluate model with scalerd dataset
        pipeline = []
        pipeline.append(('scaler', StandardScaler()))
        pipeline.append(('estimator', KerasRegressor(build_fn=baseline_model, epochs=100, batch_size=20, verbose=1)))
    ...
    def save_model(self, pipeline, folder='pcr_model'):
        ...
        pickle.dump(pipeline.named_steps['scaler'], open(f'{folder}/scaler.pkl', 'wb'))    
        pipeline.named_steps['estimator'].model.save(f'{folder}/estimator.h5')

    def load_model(self, folder='pcr_model'):
        scaler = pickle.load(open(f'{folder}/scaler.pkl','rb'))    
        ...
        return Pipeline([('scaler', scaler), ('estimator', regressor)])
